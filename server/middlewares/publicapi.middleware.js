...
module.exports = (app)=>{
    ...
    app.post(`/api/list/quote/:symbol`, async (req, res) =>{ // middleware - FIX AUTH.
        const getCryptoQuote = async () => {
            const config = {
                method: 'get',
                // baseURL: 'https://api.coincap.io/v2/assets', //coincap
                // baseURL: 'https://api.cryptowat.ch/markets/kraken/btcusd/price', //cryptowat.ch
                baseURL: `https://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest?symbol=${req.params.symbol}`, //https://stackoverflow.com/questions/20089582/how-to-get-a-url-parameter-in-express
                // baseURL: `https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest?symbol=${symbol}`,
                // url: '',
                headers: {
                // 'Api-Token': 'xxxxxxx' // default api token key:value format.
        ...
        const getQuote = await getCryptoQuote();
        res.send(getQuote);
    })
}